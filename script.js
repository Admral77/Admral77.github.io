...
const commands = {
    'help': `
        ...
        <div>- start adventure: Start a text adventure game</div>
    `,
    'ls blog': `
        <div>blog/</div>
    ...
    'clear': 'clear',
    'start adventure': `
        <div>Welcome to the Text Adventure Game! Type <strong>adventure [command]</strong> to interact with the game.</div>
    `,
    'Amelia': `
...
const adventureScript = document.createElement('script');
adventureScript.src = 'adventure.js';
document.head.appendChild(adventureScript);
...
commandInput.addEventListener('keydown', function(event) {
    if (event.key === 'Enter') {
        ...
        } else if (input === 'start adventure') {
            response = commands['start adventure'];
        } else if (input.startsWith('adventure ')) {
            const adventureCommand = input.replace('adventure ', '');
            response = adventure.handleCommand(adventureCommand);
        } else {
